...
namespace Operation{
    ...
    void MatrixOperation::writeToOutPutFileTheResultOperation() const{
        std::ofstream outFile(getOutPutFile() + "/cache", std::ios::out);
        if(!outFile.is_open()){
    ...
    void ImageOperation::writeToOutPutFileTheResultOperation() const{
        if(getOperation() == "convert"){
            bmpOperations::convert_to_grayscale(getInPutFiles(), getOutPutFile() + "/cache");
        }
        else{
            bmpOperations::rotate_image(getInPutFiles(), getOutPutFile() + "/cache");
        }
    }
